...
export const unk:Source = {name:'?'} //Used as a default when we don't know the source
...
export type ZoneName = 'supply' | 'hand' | 'discard' | 'play' | 'events' | 'void'
...
export function createRaw(state:State, spec:CardSpec, zone:ZoneName='discard', loc:InsertLocation='bottom'): [State, Card] {
    let id:number; [state, id] = state.makeID()
...
export function createRawMulti(state:State, specs:CardSpec[], zone:ZoneName='discard', loc:InsertLocation='bottom'): State {
...
export type TypedReplacer = Replacer<ResourceParams> | Replacer<CostParams> |
...
export class CostNotPaid extends Error {
...
export function gainEnergy(n:number, source:Source=unk): Transform {
...
export type SetName = 'core' | ExpansionName
